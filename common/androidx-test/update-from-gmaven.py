...
#      path
#   3. Generate METADATA files for compliance
#   4. Extract the AndroidManifest from the aars into the manifests folder
#   5. Run pom2bp to generate the Android.bp
...
metadataTemplate = """
  identifier {{
    ...
    value: "common/androidx-test/{artifactPath}"
    version: "{version}"
  }}
"""
...
def downloadAar(groupId, artifactId, version):
   ...
   _fetchArtifact(artifactPath + ".aar")
   ...
   _fetchArtifact(artifactPath + ".pom")
   _fetchArtifact(artifactPath + "-sources.jar")

   addProvenanceFile(artifactPath + "-sources.jar")
...
def downloadApk(groupId, artifactId, version):
   ...
   _fetchArtifact(artifactPath + ".apk")
   _fetchArtifact(artifactPath + ".pom")

   addMetadata(artifactPath + ".apk", version)
   addProvenanceFile(artifactPath + ".apk")

def _fetchArtifact(artifactPath):
  cmd("wget -O %s https://dl.google.com/dl/android/maven2/%s" % (artifactPath, artifactPath))
...
def addMetadata(artifactPath, version):
  with open("METADATA", "a") as f:
    f.write(metadataTemplate.format(
         version = version,
         artifactPath = artifactPath,
    ))

def completeMetadata():
  with open("METADATA", "a") as f:
    f.write("}")
...
cmd("rm -rf manifests")
cmd("cp METADATA.base METADATA")

downloadAar("androidx.test", "core", coreVersion)
downloadAar("androidx.test.espresso", "espresso-accessibility", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-core", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-contrib", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-idling-resource", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-intents", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-idling-resource", espressoVersion)
downloadAar("androidx.test.espresso", "espresso-web", espressoVersion)
downloadAar("androidx.test", "monitor", monitorVersion)
downloadAar("androidx.test", "rules", rulesVersion)
downloadAar("androidx.test", "runner", runnerVersion)
downloadAar("androidx.test.ext", "junit", extJUnitVersion)
downloadAar("androidx.test.ext", "truth", extTruthVersion)
downloadAar("androidx.test.janktesthelper", "janktesthelper", jankTestHelperVersion)
downloadAar("androidx.test.services", "storage", servicesVersion)
